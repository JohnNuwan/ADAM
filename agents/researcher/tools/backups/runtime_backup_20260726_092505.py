...
"""
import os
# === DEEPSEEK V4 — VERIFICATEUR DE CODE ===
DEEPSEEK_API_KEY = os.environ.get("OPENROUTER_API_KEY", "")
DEEPSEEK_URL = "https://openrouter.ai/api/v1/chat/completions"
DEEPSEEK_MODEL = "deepseek/deepseek-v4-flash"

def _call_deepseek(prompt, system="Tu es un expert codeur. Verifie et corrige le code. Reponds en JSON.", max_tokens=2048):
    """Appelle DeepSeek V4 Flash via OpenRouter pour verifier/corriger du code"""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=60) as resp:
            ...
            # DeepSeek V4 peut mettre la reponse dans "content" ou "reasoning"
            result = msg.get("content") or msg.get("reasoning") or ""
            return result
    except Exception as e:
        return ""

def _deepseek_verify_code(tool_name, code, skills_context="", shared_tools=""):
    """Utilise DeepSeek pour verifier et corriger le code genere par Qwen"""
    import ast, json, re
    ...
    if is_valid:
        ...
        pass_count = len([l for l in lines if l.strip() == "pass" or "TODO" in l or l.strip() == "..."])
        if total > 3 and pass_count / total > 0.4:
            is_valid = False
    
    if is_valid:
        return code, True  # Code is valid, no need for DeepSeek
    
    # Code is invalid or skeleton -> ask DeepSeek to fix it
    context = ""
    if skills_context:
        context += "\nSkills pertinents:\n" + skills_context[:500]
    if shared_tools:
    ...
    prompt = f"""Corrige ce code Python pour l'outil '{tool_name}'. Le code actuel est invalide ou incomplet (squelette avec pass/TODO).
...
Regles:
- Genere du VRAI code Python fonctionnel
- PAS de pass, TODO, ou ... comme implementation
- Inclut les imports necessaires
- Les fonctions doivent avoir de vraies implementations
- Ajoute un block if __name__ == '__main__' avec un test
- Reponds UNIQUEMENT avec le code Python (pas de markdown, pas d'explication)
"""
    ...
    try:
        ...
        return code, False  # DeepSeek failed too, return original
import logging

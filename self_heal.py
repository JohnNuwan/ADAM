...
STRATEGIES: dict[str, dict[str, Any]] = {
    ...
    "fix_handler": {
        "description": "Diagnostique et corrige un handler Adam en erreur (exit code)",
        "exit_codes": {
            126: "Permission denied → chmod +x",
            127: "Command not found → vérifier shebang/dépendances",
            1: "Erreur générale → py_compile si Python",
            2: "Argument error → vérifier argparse",
        },
    },
}
...
TYPE_TO_STRATEGY: dict[str, str] = {
    ...
    "handler_failed": "fix_handler",
    "handler_permission_denied": "fix_handler",
...
class Resolveur:
    ...
    # ── fix_handler : Docteur automatique ────────────────────────────────

    def _fix_handler(self, payload: dict, evenement: dict) -> dict[str, Any]:
        """Diagnostique et corrige un handler Adam en erreur.

        Lit l'exit code depuis le payload, applique la correction adaptée :
          - 126 (Permission denied) → chmod +x sur le handler
          - 127 (Command not found) → vérifier shebang + dépendances
          - 1   (erreur générale)  → py_compile si .py pour diagnostiquer
          - 2   (argparse error)   → log + alerte (correction manuelle)
          - timeout               → kill + restart

        Après correction, re-publie l'événement original sur le canal
        approprié pour que l'agent retente son exécution.
        """
        agent_id = payload.get("agent_id", "inconnu")
        exit_code = payload.get("exit_code")
        handler_path = payload.get("handler_path")
        stderr_snippet = str(payload.get("stderr", ""))[:500]

        # Si pas de handler_path, tenter de le récupérer depuis la DB
        if not handler_path:
            handler_path = self._lookup_handler_path(agent_id)

        if not handler_path or not os.path.isfile(handler_path):
            logger.warning(
                f"fix_handler: handler introuvable pour agent='{agent_id}' "
                f"path='{handler_path}'"
            )
            return {"succes": False, "action": "fix_handler",
                    "detail": f"handler introuvable: {handler_path}"}

        logger.info(
            f"fix_handler: agent='{agent_id}', exit={exit_code}, "
            f"handler='{handler_path}'"
        )

        # Si pas d'exit code, tenter de le déduire du stderr
        if exit_code is None:
            exit_code = self._deduce_exit_code(stderr_snippet)

        correction = None
        detail_parts = []

        # ── Dispatch selon exit code ───────────────────────────────────
        if exit_code == 126:
            # Permission denied → chmod +x
            correction = self._fix_chmod(handler_path)
            detail_parts.append(f"chmod: {correction['detail']}")

        elif exit_code == 127:
            # Command not found → vérifier shebang + dépendances
            correction = self._fix_shebang_deps(handler_path, stderr_snippet)
            detail_parts.append(f"shebang/deps: {correction['detail']}")

        elif exit_code == 1 or "SyntaxError" in stderr_snippet:
            # Erreur Python → py_compile pour diagnostiquer
            correction = self._fix_syntax(handler_path)
            ...
            correction = {"succes": False,
                          "detail": "exit code inconnu, impossible de diagnostiquer"}
            detail_parts.append(correction["detail"])

        else:
            correction = {"succes": False,
                          "detail": f"exit code {exit_code} non géré par fix_handler"}
            detail_parts.append(correction["detail"])

        # ── Si correction réussie, re-publier l'event ─────────────────
        if correction.get("succes"):
            # Re-publier l'événement original pour retry
            channel_original = evenement.get("channel", "adam:error").replace(":error", ":retry")
            self.bus.publier(
                channel=channel_original,
                payload={
                    "agent_id": agent_id,
                    "action": "retry_after_fix",
                    "fix_applied": correction.get("detail", ""),
                    "original_event_id": evenement.get("id"),
                },
                source="self-heal",
            )
            detail_final = " | ".join(detail_parts)
            logger.info(f"fix_handler SUCCÈS: {detail_final} → retry publié sur {channel_original}")
    ...
    def _deduce_exit_code(stderr: str) -> Optional[int]:
        """Tente de déduire l'exit code depuis le message stderr."""
    ...
    @staticmethod
    def _fix_chmod(handler_path: str) -> dict[str, Any]:
        """Corrige exit 126 : ajoute le bit d'exécution."""
        try:
            os.chmod(handler_path, 0o755)
            # Vérifier
            st = os.stat(handler_path)
            if st.st_mode & 0o111:
                return {"succes": True, "detail": f"chmod 755 appliqué sur {handler_path}"}
            return {"succes": False, "detail": "chmod appliqué mais bit exec toujours absent"}
        except (OSError, PermissionError) as e:
            return {"succes": False, "detail": f"chmod échoué: {e}"}

    @staticmethod
    def _fix_shebang_deps(handler_path: str, stderr: str) -> dict[str, Any]:
        """Corrige exit 127 : vérifie shebang et dépendances Python."""
        try:
            ...
            if not first_line.startswith("#!"):
                ...
                return {"succes": True, "detail": f"shebang ajouté à {handler_path}"}

            # Si shebang Python, vérifier les imports
            if "python" in first_line:
                result = subprocess.run(
                    [sys.executable, "-c",
                     f"import py_compile; py_compile.compile('{handler_path}', doraise=True)"],
                    capture_output=True, text=True, timeout=15,
                )
                if result.returncode == 0:
                    return {"succes": True,
                            "detail": f"handler compile OK, exit 127 probablement dépendance manquante: {stderr[:100]}"}
                else:
                    return {"succes": False,
                            "detail": f"py_compile échoué: {result.stderr[:200]}"}

            return {"succes": False, "detail": f"shebang non-Python: {first_line}"}
        except (OSError, IOError) as e:
            return {"succes": False, "detail": f"lecture handler échouée: {e}"}

    @staticmethod
    def _fix_syntax(handler_path: str) -> dict[str, Any]:
        """Corrige exit 1 : compile Python pour diagnostiquer l'erreur."""
        try:
            result = subprocess.run(
                [sys.executable, "-c",
                 f"import py_compile; py_compile.compile('{handler_path}', doraise=True)"],
            ...
            if result.returncode == 0:
                return {"succes": True, "detail": "py_compile OK — exit 1 non lié à la syntaxe"}
            else:
                # Extraire la ligne/erreur
                return {"succes": False,
